      ...
      <div class="sm:w-1/2 w-2/3 inline-block relative">
        <Transition name="slide-up" mode="out-in">
          <div v-if="!isStart" class="absolute mx-auto">
            <UCard>
              <template #header>
                <h1 class="font-semibold">{{ games[3].name }}</h1>
              </template>
...
const people = [
  {
    id: "1",
    label: "Сохраняющая ноль",
  },
  {
    id: "2",
    label: "Сохраняющая единицу",
  },
  {
    id: "3",
    label: "Самодвойственная",
  },
  {
    id: "4",
    label: "Монотонная",
...
async function onSubmit() {
  ...
  fun1.value = a[0].split('').sort(function(){return 0.5-Math.random()}).join('');
  fun2.value = a[1].split('').sort(function(){return 0.5-Math.random()}).join('');;
  fun3.value = a[2].split('').sort(function(){return 0.5-Math.random()}).join('');;
...
async function onAsk(event: FormSubmitEvent<any>) {
  ...
  for (let i = 0; i < selected.value.length; i++) {
    input.input += selected.value[i].id + " ";
  }
  const response = await $fetch(`http://92.53.105.185:7000/11`, {
    method: "POST",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
